  ...
  module Plugins
    module Scaffold
      module Create
        ...
        def inputs(file_path, &callback)
          html_strings       = Analyze.html_strings(file_path)
          element_attributes = Analyze.element_attributes(html_strings)
          template_cinputs   = Templates.component_inputs(element_attributes)
          path_cinputs       = Files.generate_path_cinputs(file_path)

          RJSV::Core::Files.write_with_dir(
            template_cinputs, path_cinputs, 'w'
          )

          callback.call("Created '.#{path_cinputs.sub(Dir.pwd, '')}'") if callback
        end
      end#Create
    end
  end
